...
void CPU::executeInstruction(Instruction &instruction) {
  ...
  if (m_testing) {
    ...
    test(line, instruction.menmonic());
  }
...
u8 CPU::memAccess(const u16 &address, const u8 &value, const bool &write) {
  ...
  if (address < 0x2000) { // m_RAM 0x0000 ... 0x1FFF
    ...
    return prgAccess(address);
  }

  return 0;
}

u8 CPU::read(const u16 &address) { return memAccess(address, 0, false); }
...
u8 CPU::prgAccess(const u16 &address) {
  // 0x8000 - 0xBFFF
  // 0xC000 - 0xFFFF
...
void CPU::dumpRegs() {
  ...
  // std::cout << " CYC:" << std::setw(3) << std::setfill(' ') <<
  // static_cast<int>(0) << std::endl;
  cout << std::endl;
}

void CPU::test(const string &line, const string &menmonic) {
  std::regex rgx("(.{4})\\s*(.{9}).(.{3})\\s(.{28})A:(.{2})\\sX:(.{2})\\sY:(.{"
                 "2})\\sP:(.{2})\\sSP:(.{2})\\sCYC:(.*)");
  std::smatch matches;
...
void CPU::identify(const vector<u8> &instructionData,
