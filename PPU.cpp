...
uint_least8_t PPU::access(uint_least16_t index, uint_least8_t value,
                          bool write) {
...
void PPU::rendering_tick() {
  bool tile_decode_mode =
      0x10FFFF &
      (1u << (x / 16)); // When x is 0..255, 320..335
                        // Each action happens in two steps: 1) select memory
                        // address; 2) receive data and react on it.
  switch (x % 8) {
  ...
  case 0:    // Point to nametable
    ...
    break;
  case 1:
    if (x == 337 && scanline == -1 && even_odd_toggle && reg.ShowBG)
      scanline_end = 340;
    // Name table access
    ...
    // The bitmap pattern is 16 bits, while the attribute is 2 bits, repeated 8
    // times.
...
void PPU::tick() {
  ...
  case 2:
    ...
    break;
  case 0:
    CPU::instance().nmi = reg.InVBlank && reg.NMIenabled;
    break;
  }
  if (VBlankState != 0)
