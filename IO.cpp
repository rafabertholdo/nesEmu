...
IO::IO() : joypadBits{0, 0}, joypadIndex{0, 0} {
  ...
  if (SDL_Init(SDL_INIT_VIDEO | SDL_INIT_AUDIO | SDL_INIT_JOYSTICK) < 0) {
    ...
    window = SDL_CreateWindow("Nes Emulator", SDL_WINDOWPOS_UNDEFINED,
                              SDL_WINDOWPOS_UNDEFINED, 256, 240,
          ...
          for (int p1 = 0; p1 < 64; ++p1) {
            ...
            for (int p = 0; p < 12; ++p) { // 12 samples of NTSC signal constitute a color.
              ...
              int r = (p + o * 4) % 12,
                  pixel = r < 8 - u * 2
                              ? p0
                              : p1; // Use pixel=p0 to disable artifacts.
              // Decode the color index.
              int c = pixel % 16, l = c < 0xE ? pixel / 4 & 12 : 4, e = p0 / 64;
              // NES NTSC modulator (square wave between up to four voltage
              // levels):
              int b = 40 + s[(c > 12 * ((c + 8 + p) % 12 < 6)) +
                             2 * !(0451326 >> p / 2 * 3 & e) + l];
            ...
            if (u == 2)
              palette[o][p1][p0] +=
                  0x10000 *
                  clamp(255 * gammafix(y / 1980.f + i * 0.947f / 9e6f +
                                       q * 0.624f / 9e6f));
            if (u == 1)
              palette[o][p1][p0] +=
                  0x00100 *
                  clamp(255 * gammafix(y / 1980.f + i * -0.275f / 9e6f +
                                       q * -0.636f / 9e6f));
            if (u == 0)
              palette[o][p1][p0] +=
                  0x00001 *
                  clamp(255 * gammafix(y / 1980.f + i * -1.109f / 9e6f +
                                       q * 1.709f / 9e6f));
          }
...
bool IO::CheckBit(const u8 &var, const u8 &pos) { return (var) & (1 << (pos)); }
